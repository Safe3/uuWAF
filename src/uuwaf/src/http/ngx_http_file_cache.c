...
static ngx_str_t  purge_time = ngx_string("purge_time");
static ngx_str_t  cache_time = ngx_string("cache_time");
...
ngx_http_file_cache_read(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    now = ngx_time();
	lowcase = ngx_pnalloc(r->pool, purge_time.len);
	if (lowcase == NULL) {
		return NGX_DECLINED;
	}

	hash = ngx_hash_strlow(lowcase, purge_time.data, purge_time.len);

	var.len = purge_time.len;
	var.data = lowcase;
...
ngx_http_file_cache_valid(ngx_http_request_t *r, ngx_array_t *cache_valid, ngx_uint_t status)
{
	...
	lowcase = ngx_pnalloc(r->pool, cache_time.len);
	if (lowcase == NULL) {
		return 0;
	}

	hash = ngx_hash_strlow(lowcase, cache_time.data, cache_time.len);

	var.len = cache_time.len;
	var.data = lowcase;
	vv = ngx_http_get_variable(r, &var, hash);
	if (vv == NULL) {
		return 0;
